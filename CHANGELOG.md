...
- **无正例时注入合成 few-shot** — 回贴只能回贴真实存过的块；冷启动或前几轮全漏时历史里仍然全是「没有块的助手回复」，产出仍取决于上下文里第一条有没有成功过。现在历史里一条真实块都没有时：已有助手消息则只在最近一条末尾贴格式正例，冷启动则前置一对合成对话（`MemoryOpsProtocol.formatSeed`，内容与协议说明里的 JSON 示例一致）。有真实块时仍优先回贴真实的。
...
- 新增 `ConversationManagerTest`、`MemoryManagerTest` 与 `TimeContextTest`；合成 few-shot 补了冷启动 / 全漏 / 真实块优先等用例，单测共 106 条。
